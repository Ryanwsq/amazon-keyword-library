...
锁定分类工作簿名称/哈希、Sheet2 F1–F3唯一Keyword_ID人口、站点、最新完整月、至少24完整月查询范围、来源优先级、选定提供商、趋势批次和规则版本。F4/F5/Sheet3/Sheet4及SKU事实不输入。
...
人口等于Sheet2 F1–F3；每词由同一选定提供商精确查询且至少覆盖24完整月；月度36行、季度12行；每词在两矩阵各一列；两图理论序列均为关键词数、引用实际搜索量正确范围且百分比序列为零；公式错误为零；Sheet和两图完成渲染目视复核。
