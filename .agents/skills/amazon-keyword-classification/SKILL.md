...
锁定的第二板块四Sheet、哈希、Sheet2/3/4人口、Keyword_ID、ABA/搜索量、站点、周期、分类版本和输出目录。SKU事实卡不是分类输入。
...
4. 为Sheet2在原十三列后追加`流量层、长尾主分组标签、LT分组、分类状态`。
...
9. 验证人口、主键、F1–F5、动态列清单/顺序、F5唯一主分组、LT<=20、否词来源和禁止字段；渲染所有输出Sheet。

## 质量标准

- Sheet2和Sheet4人口分别等于上游，三去向身份不变。
...
主键/人口/版本无法锁定时停止。ABA缺失不伪造流量层；无法稳定分类时使用动态`其他/未知`并标记复核。否词语义或复核状态不满足时保留在Sheet3，不强行收录。
