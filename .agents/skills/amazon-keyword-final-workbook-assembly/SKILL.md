...
5. 只从总表品类相关行生成`品类产品通用词库`的流量层与动态语义三列块，不重新判断。
...
- 总表人口等于第一板块词池，三去向全量、主键唯一、固定50列+N动态列。
- 通用词库可从总表机械复算；竞争、趋势、词频和否词人口符合范围。
...
上游哈希/版本/人口不闭合、动态列不一致、最终Sheet或顶层对象不符、外链/宏/公式错误、过程文件缺失或QA失败时阻断交付。本Skill不重算上游业务结果，不补拉数据，不生成广告判断。
