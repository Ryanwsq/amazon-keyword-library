...
锁定第二板块工作簿名称和SHA-256、Sheet2唯一Keyword_ID人口、唯一英文关键词列、词频规则版本，以及固定英语介词白名单`EN_PREP_CORE_V1`的版本、48个唯一token和内容SHA-256。只读取Sheet2非空英文关键词，不覆盖源文件。版本、数量、内容或哈希任一不一致即停止。
...
记录输入名称/哈希/人口、分词规则、介词清单/版本/token数量/内容SHA-256、原始token数、按白名单token分别统计的过滤次数、过滤介词总数、有效单词总数/唯一数、双词总数/唯一数、保留词抽查、公式/渲染、输出哈希、状态和唯一问题文档。
