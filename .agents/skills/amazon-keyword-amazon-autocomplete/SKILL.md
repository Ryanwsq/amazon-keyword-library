...
锁定的 `Run_ID`、唯一主锚点、站点 US、真实品类功能/配置探针、规格敏感性判断、本机忽略输出目录，以及首选 Codex 内置浏览器或获准普通 Chrome 备用入口。
...
4. 每个输入等待下拉建议稳定，只读取当次可见建议和顺序；不按 Enter、不进入结果页、不递归扩展新词、不使用`how/what/why`。普通行、卡片和组内选项都必须完整可见；选项被轮播控件遮挡时，在输入不变的前提下用轮播箭头或横向滚动使其完整显示，并保存操作前后证据。仍无法完整显示的选项只记异常，不进入来源事件；状态提示数量不能替代可见文本。
