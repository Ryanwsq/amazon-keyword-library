...
首选Codex内置浏览器；当前设备无法稳定识别或操作时允许切换普通Chrome。两种入口都必须保持Amazon US未登录、Department=`All`、邮编`10001`并保存同级可见证据。不得用脚本、网页搜索、搜索结果页或API替代；两个入口都不可用时必须记`not_executed`并回传主任务。被轮播控件遮挡的选项须先在输入不变时尝试完整揭示，仍未完整可见则只记异常，不进入来源事件。
