...
Amazon联想是必选来源，固定使用Codex内置浏览器、Amazon US未登录、All、10001；不可操作时必须`not_executed`且正常案例未完成。第二板块按动态完整商品对象分流，不按SKU配置删除同品类词；拼写错误、其他语言和自有品牌不得一律摘除。分类F5按唯一主标签分组且每词只出现一次。竞争正式输入只有SIF Top3点击份额与Top3转化份额；任一项缺失、冲突未解决或周期不明时不出综合等级，不再使用机会筛选、竞争格局、卖家精灵竞争字段、比较池或样本门。趋势月份为空时不生成图。P1验收未完成时不得冒充已完成或已验证能力。

## Status integrity

当前唯一清洗基线仍是V2.1；V2.2只标识词频组件。2026-08-20回写和2026-08-21输出合同确认均为Post-V2.1增量，不命名为新清洗版本。旧工作簿在规则修正前生成，只能作为旧运行证据；补齐联想并按新规则完整重跑后才能继续验收。最终交付固定为一个过程文件夹和一个七Sheet最终工作簿；最终总表覆盖三去向全人口，否词库只保留语义否词类别且不含否定方式，品类通用词库只从最终总表机械筛选。广告资格、否定匹配方式和投放动作仍为后置。来源采集已拆为SIF、Amazon联想和卖家精灵三个单一职责Skill，竞争与趋势已拆分，并建立独立质量验证Skill；当前共十二个Skills，全部保持`draft`且能力为`planned`，本次合同同步和P0通过不生成P1证据。
