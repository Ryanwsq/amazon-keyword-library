...
- 2026-08-22已确认“主执行锚点、已确认核心词/等价表达和完整词中心对象”必须分层锁定：较具体的主锚点不得降级较宽泛但证据闭合的核心词，已确认等价表达也不得重新当普通相邻商品判断；完整短语出现决定性语义并改变中心商品时，核心词根仍不得覆盖完整词判断。配置连接、并列连接和其他语言词序统一按中心购买对象整批判断并反向抽查。以上均为Post-V2.1边界澄清，仍非P1。
...
| 卖家精灵扩词 | 四列业务表；有界最大召回；MCP首选、同提供商网页备用 | draft/planned；待正常案例P1 |
| 三来源机械合并 | 两Sheet：关键词池、任务信息与类目锚点 | 合同已确认，待新Run重跑 |
| 品类清洗 | 四Sheet：事实与锚点、品类相关、其他摘除、二类词 | V2.1边界不变；已补充核心词/等价表达优先级、可靠变体属性门与完整词覆盖门，待重跑验证 |
| 分类 | Sheet2追加4固定列+N动态语义列；Sheet4追加4固定列；最小否词库 | 合同已确认，待P1 |
| 词频 | Sheet2英文词；`EN_PREP_CORE_V1`固定48-token排除与双词断点；两张三列表 | 有界机械验证通过；非完整来源、无P1；draft/planned |
| 竞争 | Sheet2 F1–F4；只用SIF Top3点击/转化份额；12列 | 合同已确认，待P1 |
| 趋势 | Sheet2 F1–F3；`SellerSprite -> Sorftime`且单Run单源；至少24完整月；月/季实际搜索量图 | 合同已确认；当前仅边界候选，待正常案例P1 |
...
- 在锁定的当前revision上启动新的完整正常案例Run，按当前来源备用入口、完整词规则、趋势单源与实际搜索量图合同从头执行；不得沿用规则迭代前的受影响判断结果。
