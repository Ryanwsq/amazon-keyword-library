...
1. SIF回传后，主任务先确认唯一主锚点和1–3个代表种子；随后Amazon联想与卖家精灵扩词并行，三来源机械合并等待两者和SIF全部回传。
2. 清洗完成Sheet2/3/4唯一去向和行数闭环后，词频统计与关键词分类并行。词频不参与分类判断；最终工作簿装配等待词频分支和第三板块分支全部完成。
3. 关键词分类完成后，竞争性分析与趋势性分析并行；最终工作簿装配等待两者及分类派生表全部回传。

并行任务各自记录输入版本、异常和完成状态；不得把一个分支完成推断成整组完成。
...
最终工作簿装配完成后，主任务必须把锁定的 Run、仓库 revision、阶段清单、合同版本和本机产物路由给 `keyword-quality-reviewer`。质量副任务只读检查并返回`pass`、`blocked`或`incomplete`；不得修改上游文件、补造证据或调用业务外部系统。没有独立质量回传时，主任务不得把第一次正式只读验证标记完成。
