...
- Synchronization note: 已同步2026-08-20单一职责拆分、本机长期任务拓扑、用户确认的并行调度和独立私有GitHub基线：SIF完成并确认锚点/种子后Amazon联想与卖家精灵扩词并行；清洗闭环后词频与关键词分类并行；分类完成后竞争与趋势并行；机械合并和最终装配分别等待适用分支汇合。十个长期副任务已在本设备的独立Codex项目中创建，真实映射只留Git忽略文件；`main`是唯一长期基线分支。当前共十二个draft Skills，P0结构校验通过，能力仍均为planned且没有新增P1证据。已复核，无业务流程、规则、阈值或V2.1/V2.2基线变化
...
flowchart TB
    ...
    subgraph P3["第三板块｜关键词分类、竞争性、否词与趋势分析"]
        direction TB
        P3A["锁定第二板块版本、第一板块卖家精灵原始数据版本<br/>Sheet2/3/4行数、数据周期和稳定Keyword_ID<br/>词频并行产生且不参与分类；SKU事实卡不进入第三板块"]
        ...
        P3H["只选F1–F4建立独立竞争Sheet<br/>SIF机会筛选精确值正式；竞争格局辅助<br/>卖家精灵复用SPR/商品数量；样本不足10不出综合等级"]
...
| 第三板块：分类与分析 | 第二板块版本、第一板块卖家精灵原始数据、Sheet2/3/4、周期和主键；不输入SKU事实卡；Sheet5词频可并行产生 | F5唯一主标签分类、独立二类词/否词候选/竞争Sheet和卖家精灵F1–F3月度趋势Sheet | 样本不足10不出竞争等级；月份为空不出趋势；新规则未重跑，Skills无P1 |
...
|---|---|---|---|---|---|
| 竞品流量反查 | SIF MCP | `keyword.source.competitor-traffic.query` | 站点、逐个获准竞品 ASIN、最近 30 天 | 竞品流量关键词、全部原始字段、查询和覆盖状态 | 本机已由用户确认连接；不重复做连通性验证 |
...
| 大范围扩词 | 卖家精灵 MCP | `keyword.source.keyword-mining.query` | 站点、1–3 个代表种子、页码和必填参数 | 每页全部关键词和全部返回字段 | 本机已由用户确认连接；不重复做连通性验证 |
...
| 关键词竞争格局 | SIF MCP | `keyword.competition.sif-landscape.query` | 站点、单个关键词和周期；当前不传目标ASIN | 集中度历史、结构分化、Top3外购买空间、头部渠道结构和可进入性解释 | `planned`；广告建议和目标ASIN位置不进入 |
| 关键词机会筛选 | SIF MCP | `keyword.competition.sif-opportunity.screen` | 站点、获准词根和返回数量 | 完整关键词精确匹配的 CPC、Top3点击/转化、市场CVR、入场信号、Top3 ASIN和周期 | `planned`；不调用词根维度竞争者识别 |
...
| 竞争性分析 | F1–F4独立竞争、精确正式值和严格样本门已确认，已拆为独立draft Skill | 样本不足10不出综合等级；新规则未重跑且Skill无P1 |
...
- 公司原版 V2.1 的固定执行参数是 SIF 每个 ASIN 取接口上限 300 条、卖家精灵每页 20 条。实际返回结构若与这两个已确认参数冲突，记录异常并停止修改口径，不能静默把新数值写成业务规则。

### MCP 发生异常时

- SIF 查不到数据：检查站点、父子体和 ASIN 状态；记录失败，不用未经用户确认的相似 ASIN 替换。
- 卖家精灵 MCP 明确报错、限流、返回结构异常、缺页、整页重复、循环页或关键字段结构性缺失：立即停止，不继续其他页或种子，返回 `MCP返回数据有误`；单行指标空白只记录为字段缺失，已取得数据保留作排障证据。
...
还需要：锁定的第二板块工作簿版本、第一板块 `06_卖家精灵原始数据`版本、Sheet2/3/4 原始行数、稳定 `Keyword_ID`、类目锚点卡、站点和数据周期。SKU 事实卡不进入第三板块；`Sheet5_词频统计`可与分类并行产生，只作为血缘和最终过程 Sheet 保留，不参与第三板块判断。最终装配前缺少应有词频结果时回到词频 Skill 补齐。
...
| 第三板块：分类与竞争/趋势 | 步骤 5 | 第二板块 Sheet2/3/4、第一板块卖家精灵原始数据和稳定主键；词频分支并行 | ABA F1–F5、词性/词义标签、否词候选、F1–F4独立竞争 Sheet、品类 F1–F3 历年月度搜索量折线图；不使用词频做判断 | Sheet1 最终决策总表、词频/竞争/Sheet3/Sheet4/趋势独立 Sheet、全部过程 Sheet |
...
5. 每个 ASIN 单独保存全部返回行和全部原始字段，再汇总；不因低流量、重复或看起来无关而删除。重试或补拉必须形成新版本调用，不覆盖旧响应。
6. SIF 同时承担两件事：提供正式原始关键词来源；用真实流量表达提出一级品类核心大词候选。它不负责穷尽所有关键词，也不负责确认 SKU 精准细分类型。

至少能够看回：竞品 ASIN、关键词原文、流量份额、ABA 排名、月搜索量、自然流量占比、自然排名、SP/SB/SBV 等广告位置、渠道覆盖、流量变化、查询周期和抓取时间；接口返回的其他字段也不能丢。
...
1. 对去重后的 1–3 个代表种子逐个执行关键词挖掘。
2. 每个种子从第 1 页开始，以公司版 V2.1 已确认的每页 20 条连续获取，直到接口明确最后一页、总页数完成或下一页为空。
3. 每页每行和所有返回字段完整保存，不按搜索量、ABA、相关性或词长过滤；页边界重复在原始表中保留，进入总表时再按机械匹配键合并成一条关键词。
4. 接口声明总数与实际稳定分页行数不一致时，以实际返回行数为准并记录总数漂移。只有页码连续、存在明确最后页、无缺页、无整页重复和无循环页，才能继续；否则停止并写异常。
5. 同一规范关键词的重复行若指标冲突，总表只展示首次出现值，全部原始行仍保留用于追溯；不得平均、挑选或覆盖。
6. 只有所有代表种子都到明确结束信号，才能写“接口范围内已完整”，不能写“Amazon 全量关键词”。
...
4. 使用稳定 `Batch_ID`、`Keyword_ID` 和三类来源记录 ID 建立跨表追溯。
5. 规范 ABA 排名和规范搜索量优先取卖家精灵；仅存在于 SIF 的关键词才回退到 SIF。保留每个来源原值、数据周期、规范值来源和是否回退，不做跨来源平均。
6. SPR和商品数量只取卖家精灵；来源完成度与逐词指标完整度分开记录，缺值保持空白并写缺失来源，不能因整批来源完成就假设每个词字段齐全。
...
| `04_SIF原始数据` | 各竞品最近 30 天的逐条原始记录和全部字段 |
| `05_搜索框联想` | 每个输入串触发的全部可见建议、环境和状态 |
| `06_卖家精灵原始数据` | 各代表种子从第一页到结束的逐页原始记录和全部字段；第三板块正式读取 SPR、商品数量，CPC和集中度只作原始溯源 |
| `07_关键词来源明细` | 每条原始来源记录的统一索引 |
...
- 卖家精灵明确报错、限流、结构异常、缺页、整页重复、循环页或关键字段结构性缺失：立即停止，不继续其他页或种子，返回`MCP返回数据有误`；单行指标空白只记录为字段缺失，不把它误判成整批失败。
...
5. 所有代表种子都从卖家精灵第 1 页拉到明确结束，页码连续且无缺页、整页重复或循环页；每页、每行和每个字段均已保留，总数漂移已按实际行数记录。
6. 每个 `Keyword_ID` 至少能回到一条来源明细，来源明细又能回到三张原始来源表之一。
7. 三类来源数量、来源明细数量和机械去重后的关键词数量已经核对。
8. 没有执行语义去重、相关性删除或流量分类；出现卖家精灵明确报错时，工作簿只能作为排障文件，不能进入步骤 3。
9. 规范 ABA/搜索量的来源优先级、SIF回退状态、卖家精灵 SPR/商品数量缺失和逐词指标完整性均已单独记录；没有跨来源平均或伪造缺值。
...
```mermaid
flowchart TD
    A["输入锁定：第二板块版本 + 第一板块卖家精灵原始数据版本 + Keyword_ID + Sheet2/3/4行数 + 类目锚点卡 + 数据周期<br/>词频分支并行且只作血缘；SKU事实卡不进入第三板块"]
    ...
    E --> K["只选择F1–F4做竞争<br/>SIF机会筛选 + 单词竞争格局；不调用词根竞品接口<br/>卖家精灵复用第一板块SPR、商品数量"]
    K --> L["独立竞争性分析Sheet<br/>机会筛选精确词提供CPC/Top3点击与转化/CVR/入场信号；竞争格局仅辅助<br/>样本池仍少于10时不出综合等级"]
...
2. 锁定第一板块 `06_卖家精灵原始数据`批次版本，以及类目锚点卡、站点和数据周期；关键词分类可与 `Sheet5_词频统计`并行。词频只用于血缘和过程 Sheet 保留，不参与本板块判断，SKU 事实卡不进入第三板块。
...
2. SIF 只使用 `market_screen_keyword_opportunities` 和 `market_get_keyword_competition`；不使用 `market_get_keyword_root_competitors`。`market_screen_keyword_opportunities` 的完整关键词精确匹配值是正式来源；`market_get_keyword_competition`只提供竞争格局辅助证据，当前不传目标 ASIN。
3. 正式判断字段来自机会筛选的 SIF CPC、Top3 ASIN点击份额、Top3 ASIN转化份额、市场 CVR和入场信号，以及第一板块卖家精灵的 SPR、商品数量。卖家精灵 CPC和点击集中度只可保留为过程溯源，不覆盖 SIF 正式值。
4. 竞争性步骤不重新调用卖家精灵关键词挖掘、不按精确词补拉，也不局部刷新。SPR或商品数量缺失时标记`第一板块数据缺失`；需要刷新周期时重跑版本化的第一板块采集批次。
5. Top3 点击与转化分别按 `<30%`低、`30%–<50%`中、`50%–<70%`高、`>=70%`极高；两者相差至少 15 个百分点时标记结构分化。
6. CPC、SPR、商品数量和市场 CVR 在同站点、同品类、同周期、同流量层内使用 P33/P67。层内有效词少于 10 个时依次回退到 F1+F2 或 F3+F4、全部 F1–F4；回退后仍不足 10 个时只保留原始值和可独立成立的维度，综合等级、综合判断和行动含义统一写`数据不足/人工复核`。
7. 综合竞争等级按极高、高、中、低有序判断：头部锁定、SPR、CPC、商品数量是四个竞争维度，市场 CVR只作最多一级修正。SIF 入场信号与项目等级相差两级以上且无法解释时进入人工复核。
8. Top3 点击或转化缺失、CPC/SPR/商品数量缺失两项以上、机会筛选未精确匹配、比较池回退后少于10、周期方向冲突或非 F1–F4时，不输出正式等级。机会筛选没有完整词精确匹配时，不得用词根、近义词或相关词替代。

独立竞争 Sheet 保存分来源周期、正式输入、精确匹配与比较池样本、集中度/Top3 ASIN等过程证据、派生等级、综合竞争等级、判断摘要、数据完整性、置信度和人工复核状态。详细判断见判定边界与 Skill 合同。竞争性分析不输出直接打、测试、暂缓、广告资格或广告结构。
...
Sheet1 最低字段为：`Keyword_ID`、关键词、中文翻译、标准化关键词、站点、来源渠道、采集批次、数据周期、ABA 排名、搜索量、SPR、流量层代码、流量层名称、LT 分组、词性/词义多标签、`Competition_Record_ID`、综合竞争等级、竞争判断摘要、竞争数据完整性、人工复核状态和规则版本。详细竞争字段与趋势都不写入 Sheet1；F5 标记`不适用（F5）/范围外`，人工复核状态汇总分类和竞争结果。
...
- 第三板块规范 ABA 缺失或不可比、SIF 机会筛选无完整词精确匹配、竞争比较池回退后仍少于10，或卖家精灵趋势月份为空/不可解析。
...
- 竞争性只对 Sheet2 F1–F4 使用机会筛选完整词精确匹配的 SIF CPC、Top3 ASIN点击/转化份额、市场 CVR和入场信号，以及第一板块已采集的卖家精灵 SPR、商品数量；竞争性步骤不重新调用卖家精灵关键词挖掘。
- SIF 只调用关键词竞争格局与关键词机会筛选两个接口，不调用词根维度竞争者识别；竞争格局仅作辅助，不能覆盖机会筛选正式值。
- 竞争明细独立成 Sheet，按 Top3 绝对阈值、同层 P33/P67和有序综合规则输出竞争等级；无完整词精确匹配或比较池回退后仍少于10时不输出综合等级，统一`数据不足/人工复核`。
...
| 环节 | 当前状态 | 已经完成 | 还缺什么 |
|---|---|---|---|
| 项目结构、知识分层与长期任务 | ✅ 已建立；P0通过 | 稳定知识、历史证据、完整版本决策链、当前状态和判定边界已分开；十二个draft Skill包完成结构校验；十个长期副任务已在本设备创建并写入忽略映射；私有GitHub `main`固定基线已建立 | 尚无新增P1证据；首次正式只读验证待用户提供新真实案例 |
| 步骤 0：版本、任务编排与来源合并 | 🟡 draft/planned；P0通过 | operations Skill拥有路由、阶段门、三来源机械合并与十表装配合同 | 尚未在固定基线和新真实案例上完成P1 |
| 步骤 1–2：来源采集 | ✅ 业务流程已确认；🟠 首轮正常案例进行中；三个Skills未验证 | SIF竞品反查、Amazon联想、卖家精灵扩词已拆为三个单一职责draft Skills；规范指标优先级、分页异常、固定联想环境和1–3个种子已确认 | 内置浏览器可拉起但无法稳定识别/操作，联想保持`not_executed/待解决`；三个来源Skills及主任务合并合同均无P1三案例 |
...
| 步骤 5B：竞争性 | 🟡 边界与合同已确认；独立draft Skill未验证 | F1–F4独立竞争 Sheet；机会筛选精确词正式值、竞争格局辅助值、卖家精灵SPR/商品数量；比较池不足10阻断综合等级 | 本轮旧竞争等级须按严格门重跑；P1待完成 |
...
当前首轮正常案例已暴露并确认来源、清洗、F5分组、竞争样本门、趋势月份和工作簿视觉门等问题。内置浏览器联想仍为`not_executed/待解决`，旧清洗与竞争结果尚未按新规则重跑，因此本案例不能写成完成、边界/异常案例或 P1 通过。详细问题与处理状态见最新项目交接和问题台账。

## 18. 下一步应该做什么

下一项单一职责工作是：**在保留内置浏览器技术问题为待解决项的前提下，补齐联想后按新边界重跑首轮正常案例，并继续用真实案例迭代 Sheet3 否词边界。**

建议顺序：

1. 后续单独解决 Codex 内置浏览器可拉起但无法稳定识别/操作的问题；解决前不得用外部 Chrome、API或其他来源替代必选联想。
2. 联想补齐后，从锁定输入重跑首轮正常案例，并输出清洗`旧去向 → 新去向`和竞争等级差异。
...
| `.agents/skills/amazon-keyword-competition-analysis/` | Sheet2 F1–F4竞争证据、精确机会值、比较池与独立竞争Sheet |
...
| 01.7 卖家精灵MCP分页与错误规则 | 卖家精灵扩词Skill、能力清单、本文步骤2D | 已落地；按每页20条连续分页，以实际行数为准，页边界重复原始保留、总表机械合并，缺页/整页重复/循环页停止 |
| 01.8–01.13 三类来源合并、机械键、十表和交付门 | operations Skill的来源合并合同、本文步骤2E–2F | 已落地；主任务只做机械合并，原始字段、多来源、主键和异常日志均保留 |
...
| 03 关键词分类、竞争性、趋势性与最终工作簿 | 稳定知识、`PROJECT.md`、分类Skill、边界停止门、独立竞争Skill、独立趋势Skill、最终工作簿Skill、本文步骤5 | F5主标签组内LT、机会筛选精确值、样本不足阻断等级、趋势月份门和工作簿视觉门已分别落入draft Skills；否词与P1未完成 |
...
1. `amazon-keyword-sif-competitor-collection`：逐个反查获准竞品并即时落盘完整SIF响应。
2. `amazon-keyword-amazon-autocomplete`：只用固定Codex内置浏览器环境采集Amazon联想矩阵。
3. `amazon-keyword-sellersprite-expansion`：对1–3个代表种子执行卖家精灵连续分页扩词。
